...
public class robot2Script : MonoBehaviour
{
    ...
    void Update()
    {
        if (Mathf.Abs(ARCamera.position.magnitude - transform.position.magnitude) > 0.5)
        {
            Vector3 aux = new Vector3(ARCamera.position.x, 0f, ARCamera.position.z);
            transform.position = Vector3.MoveTowards(transform.position, aux, .03f);
        }
    }
